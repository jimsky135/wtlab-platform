...
export const en = {
	...
	results: {
		inventoryBufferCheck: {
			...
			fields: {
				...
				inTransitQuantity: { label: 'In-Transit Quantity' },
				arrivalTime: { label: 'Expected Arrival Time' },
				unit: { label: 'Unit' },
		...
		leadTimeGapChecker: {
			...
			fields: {
				...
				currentStock: { label: 'Current Stock' },
				monthlyConsumption: { label: 'Monthly Consumption' },
				leadTime: { label: 'Supplier Lead Time' },
				safetyBuffer: { label: 'Safety Buffer' },
				currentDate: { label: 'Current Date', optional: 'defaults to today' },
				unit: { label: 'Unit' },
				period: { label: 'Period' },
				beginningInventory: { label: 'Beginning Inventory' },
				supplierLeadTimeMonths: { label: 'Supplier Lead Time (months)', optional: 'optional' },
				safetyBufferMonths: { label: 'Safety Buffer (months)', optional: 'optional' },
				consumption: { label: 'Consumption' },
				arrivalQuantity: { label: 'Arrival Quantity', optional: 'optional' },
			},
			buttons: {
				runQuick: 'Run Quick Check',
				runAdvanced: 'Run Planning Analysis',
			...
			labels: {
				uploadCsvNote: 'Upload a previously downloaded {filename} (or the filled blank template). Files are read in this browser only.',
				manualTableNote:
					'Beginning inventory, supplier lead time, and safety buffer are per item (first row). Consumption and scheduled arrivals are per period.',
				coverage: 'Coverage',
				noConsumption: 'n/a — never depletes',
				estimatedDepletion: 'Estimated depletion date',
				bufferFloor: 'Safety buffer reached',
				expectedArrival: 'Expected arrival (if ordered today)',
				gap: 'Gap',
				gapYes: 'yes',
				gapNo: 'no',
				...
				shortage: 'SHORTAGE',
				belowBuffer: 'below buffer',
				gapWindowLabel: 'gap window',
				ok: 'ok',
				expectedArrivalPeriod: 'Expected arrival period',
			},
			risk: {
				safe: { label: 'Safe', description: 'No timing concerns at the current consumption rate and lead time.' },
				warning: { label: 'Warning', description: 'Timing is tight — small delays could turn this into a real gap.' },
				'gap-risk': { label: 'Gap Risk', description: 'The safety buffer no longer covers this lead time.' },
				'critical-gap': { label: 'Critical Gap', description: 'The most severe timing outcome this instrument tracks — a real stockout.' },
			},
		},
		bufferDriftMonitor: {
			modeLabel: {},
			fields: {
				itemName: { label: 'Item Name', optional: 'defaults to item-1' },
				monthlyConsumption: { label: 'Monthly Consumption' },
				intendedBufferMonths: { label: 'Intended Buffer (months)' },
				actualBufferQuantity: { label: 'Actual Buffer Quantity' },
				period: { label: 'Period' },
			},
			buttons: {
				runQuick: 'Run Quick Check',
				runAdvanced: 'Run Tracking Analysis',
				downloadInput: 'Download Input CSV',
				downloadTemplate: 'Download Blank Template',
				downloadResult: 'Download Result CSV',
				addItem: 'Add Item',
			...
			labels: {
				uploadCsvNote: 'Upload a previously downloaded {filename} (or the filled blank template). Files are read in this browser only.',
				manualTableNote: 'Intended buffer is per item (first row). Monthly consumption and actual buffer quantity are observed per period.',
				intendedQuantity: 'Intended buffer (quantity)',
				actualMonths: 'Actual buffer (months)',
				noConsumption: 'n/a — no consumption',
				driftMonths: 'Drift (months)',
				driftQuantity: 'Drift (quantity)',
				driftPercent: 'Drift (%)',
				notApplicable: 'n/a',
				reasons: 'Reasons',
				period: 'Period',
			...
			status: {
				'on-target': { label: 'On Target', description: 'Actual buffer matches the intended policy within tolerance.' },
				'under-buffered': { label: 'Under-Buffered', description: 'Actual buffer is running below the intended policy.' },
				'severely-under-buffered': { label: 'Severely Under-Buffered', description: 'Actual buffer is materially below the intended policy.' },
				'over-buffered': { label: 'Over-Buffered', description: 'Actual buffer exceeds the intended policy.' },
				'severely-over-buffered': { label: 'Severely Over-Buffered', description: 'Actual buffer materially exceeds the intended policy.' },
			},
			trend: {
				widening: 'Widening',
				narrowing: 'Narrowing',
	...
	messages: {
		...
		LEAD_TIME_CURRENT_DATE_DEFAULTED: 'Current Date is blank — today will be used.',
		LEAD_TIME_CURRENT_DATE_INVALID_ISO: '"{field}" must be an ISO date (YYYY-MM-DD), got "{value}".',

		// Lead Time Gap Checker engine narratives
		LEAD_TIME_WARNING_THIN_MARGIN: 'Replenishment would arrive with little margin before the safety buffer is reached.',
		LEAD_TIME_WARNING_GAP_RISK: 'Replenishment would arrive after the safety buffer is expected to run out.',
		LEAD_TIME_WARNING_CRITICAL_GAP: 'Replenishment would arrive after stock is expected to fully run out.',
		LEAD_TIME_ACTION_SAFE: 'No action needed — timing has adequate margin.',
		LEAD_TIME_ACTION_WARNING: 'Margin is thin — confirm the order will go out on time or consider expediting.',
		LEAD_TIME_ACTION_GAP_RISK: 'Place the order now or expedite — the safety buffer will likely be breached before replenishment arrives.',
		LEAD_TIME_ACTION_CRITICAL_GAP: 'Expedite immediately or arrange interim supply — stock is expected to run out before replenishment arrives.',

		// Adapters — structural guards (Buffer Drift Monitor)
		ITEM_MISSING_INTENDED_BUFFER: 'Item "{name}" has no intended buffer declared on any of its rows.',

		// Buffer Drift Monitor engine narratives
		BUFFER_DRIFT_WARNING_UNDER: 'Actual buffer is running below the intended policy.',
		BUFFER_DRIFT_WARNING_SEVERE_UNDER: 'Actual buffer is materially below the intended policy — stockout risk is elevated.',
		BUFFER_DRIFT_WARNING_OVER: 'Actual buffer exceeds the intended policy.',
		BUFFER_DRIFT_WARNING_SEVERE_OVER: 'Actual buffer materially exceeds the intended policy — capital may be tied up unnecessarily.',
		BUFFER_DRIFT_ACTION_ON_TARGET: 'No action needed — keep monitoring.',
		BUFFER_DRIFT_ACTION_UNDER: 'Review replenishment cadence or safety stock to close the gap.',
		BUFFER_DRIFT_ACTION_SEVERE_UNDER: 'Increase safety stock or replenishment frequency now.',
		BUFFER_DRIFT_ACTION_OVER: 'Review whether the excess is intentional; consider reducing replenishment.',
		BUFFER_DRIFT_ACTION_SEVERE_OVER: 'Reassess the buffer policy or reduce inbound quantity.',
	},
