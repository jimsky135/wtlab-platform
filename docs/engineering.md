...
| Shared session dataset | one upload feeding several instruments until the session ends. Blocker is semantic, not storage: same field id means different things across instruments. Findings in [shared-session-dataset.md](shared-session-dataset.md); **HOLD until explicitly started** |
