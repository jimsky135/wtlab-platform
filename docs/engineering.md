...
| Shared session dataset | one upload feeding several instruments until the session ends. Blocker is semantic, not storage: three fields have inverted required-ness and 0-handling across tools. Full field audit, compatibility matrix and verdict in [shared-session-dataset.md](shared-session-dataset.md); **HOLD until explicitly started** |
| Pages unknown-route fallback | Cloudflare Pages answers any unmatched path with **200 + the homepage** instead of 404 (the build ships no `404.html`), so a retired route like `/workspace/entity` and any typo silently render the homepage. Knock-on: the Integrated Workspace guest panel reports "could not reach" rather than "API not running here", because it keys on `status === 404` and Pages never sends one. Pre-existing Pages behaviour, not introduced by ADR-0004 — the prototype Worker is already correct via `not_found_handling: "none"`, so promoting that Worker would dissolve the issue on its own. Two candidate fixes: add a `404.astro` (root cause) or loosen the panel check to "response was not JSON" (symptom only). **HOLD — handle separately** |
| Continuity page Export status | The Continuity page still lists **Export** as planned and says none of its capabilities are functional, but result CSV download already works on six instruments (Water Level also offers JSON). Instrument result export and a Continuity-level export package are kept as separate meanings; the site header was narrowed to "Save — planned" on 2026-09-11 (`8184b59`), the Continuity page was deliberately left untouched. Fixing it means changing `ContinuityAction.state` handling and the always-"planned" tag in `ContinuityView`, not just copy. **HOLD — decide with the Continuity work** |
| Guest login on `www.wtlab.co` | ✅ **Resolved 2026-09-10 — narrow route selected.** `www.wtlab.co/api/guest/*` → existing Worker `wtlab-guest-workspace-prototype` (persisted in `wrangler.jsonc`); Pages continues to serve every normal frontend page — no whole-site Worker conversion. Production `guest / guest` login PASS; minimum production validation 7/7 PASS. Details in [deployment.md](deployment.md#prototype-resources--live-path-routed-in-production). Relation to the Pages unknown-route fallback above: its guest-panel knock-on no longer applies on `www.wtlab.co`, because `/api/guest/*` is now answered by the Worker; the broader fallback (every other unmatched path → 200 + homepage) is unchanged and stays HOLD. Repeated authenticated login now reuses the existing valid session; production verified 2026-09-11. Anonymous pre-login rows and abandoned sessions still await a scheduled TTL sweep (see deployment.md) |
